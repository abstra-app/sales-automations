...
class SubscriptionsResponse:
    ...
    def from_dict(data):
        return SubscriptionsResponse(
                elements=[SponsoredLeadNotification.from_dict(element) for element in data["elements"]],
                paging=Paging.from_dict(data["paging"])
                )

client_secret = os.getenv("LINKEDIN_CLIENT_SECRET")
client_id = os.getenv("LINKEDIN_CLIENT_ID")
access_token=get_access_token("linkedin").token
...
def fetch_lead_notification_subscriptions():
    ...
    sponsored_response = requests.get(get_url + f"?q=criteria&owner=(value:(sponsoredAccount:{escaped_urn}))&leadType=(leadType:SPONSORED)", headers=headers)
...
def create_lead_notification_subscription(webhook: str):
    data = {
            "webhook": webhook,
            "owner": {
                "sponsoredAccount": sponsored_account_urn
                },
            "leadType": "SPONSORED"
            }
